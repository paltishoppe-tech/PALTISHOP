...
// fields: note, card, collar, peluche, espejo, extras14, extraPolaroids
// =========================
const packConfigs = {
  "pack-1": {
    title:"Pack 1 (Corderito)",
    fields:["note","card","collar","extraPolaroids","extras14"],
    options:{
  ...
  "pack-nochebuena": {
    ...
    fields:["note","card","peluche","espejo","collar","extras14"],
    options:{
      peluche:["Elefante","Hello Kitty","Nutria","Otro (escribir)"],
  ...
  "pack-2": {
    ...
    fields:["note","card","collar","extraPolaroids"],
    options:{
      collar:["Tulipán","Mariposa","Girasol","Otro (escribir)"],
      card:["N°1","N°2","N°3","N°4","N°5","N°6"]
    }
  ...
  "pack-3": {
    title:"Pack 3",
    fields:["note","card","peluche","collar"],
    options:{
      peluche:["Elefante","Hello Kitty","Nutria","Otro (escribir)"],
  ...
  "pack-5": {
    title:"Pack 5",
    fields:["note","card","peluche","espejo"],
    options:{
      peluche:["Elefante","Hello Kitty","Nutria","Otro (escribir)"],
  ...
  "pack-6": {
    title:"Pack 6",
    fields:["note","card","peluche","extraPolaroids","extras14"],
    options:{
      peluche:["Elefante","Hello Kitty","Nutria","Otro (escribir)"],
      card:["N°1","N°2","N°3","N°4","N°5","N°6"]
    }
  ...
  "pack-7": {
    title:"Pack 7",
    fields:["note","card","peluche","collar","espejo","extraPolaroids","extras14"],
    options:{
      peluche:["Elefante","Hello Kitty","Nutria","Otro (escribir)"],
      collar:["Girasol","Tulipán","Mariposa","Otro (escribir)"],
      espejo:["Lámpara tulipán","Nube","Tulipanes infinitos","Otro (escribir)"],
...
// Add-ons pricing
const ADDON_PRICES = {
  extras14: 14.00,          // 3 fotos + llavero
  extraPolaroids: 14.00     // (si quieres usarlo para 3 fotos + llavero también, cambia aquí)
};
...
function bindShopbar(){
  ...
  $("#openSearch").addEventListener("click", () => {
    ...
    // keep current query, but refresh list
    renderSearchResults();
    $("#searchInput").focus();
  });
...
function renderPackCustomizer(packId){
  ...
  currentPackState = { note:"", card:"N°1", collar:"Tulipán", collar_other:"", peluche:"Nutria", peluche_other:"", espejo:"Nube", espejo_other:"", extras14:false, extraPolaroids:false };
  ...
  if(cfg.fields.includes("peluche")){
    const opts = (cfg.options?.peluche || ["Nutria","Hello Kitty","Elefante","Otro (escribir)"]).map(o => `<option>${esc(o)}</option>`).join("");
    rows.push(`
      <div class="pfRow">
        <label>Peluche 🧸</label>
  ...
  if(cfg.fields.includes("extraPolaroids")){
    rows.push(`
      <label class="pfCheck">
        <input type="checkbox" id="pf_extraPolaroids">
        <div>
  ...
  const exPol = $("#pf_extraPolaroids");
  if(exPol) exPol.addEventListener("change", () => { currentPackState.extraPolaroids = exPol.checked; updatePackTotal(); });
...
function packTotal(){
  ...
  if(currentPackState.extras14) total += ADDON_PRICES.extras14 || 14;
  if(currentPackState.extraPolaroids) total += 14.00;
  return total;
}
...
$("#pkBuy")?.addEventListener("click", () => {
  ...
  const extras = [];
  if(cfg.fields.includes("extras14") && currentPackState.extras14) extras.push(`+3 polaroids + llavero (S/ ${fmt(ADDON_PRICES.extras14)})`);
  if(cfg.fields.includes("extraPolaroids") && currentPackState.extraPolaroids) extras.push("+3 polaroids + llavero (S/ 14.00)");
