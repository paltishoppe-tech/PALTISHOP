...
// ============================================================
function cardTemplate(item, kind="producto"){
    ...
    <article class="card" data-filter="${escHtml(item.filter||'all')}">
      <div class="card__img">
        <img src="${img}" alt="${escHtml(item.name)}" loading="lazy"
             onerror="this.src='page-01.webp'; this.style.opacity='.3'"/>
      </div>
...
function render(){
  ...
  $("#packsGrid").innerHTML    = packs.map(p => cardTemplate(p, "pack")).join("");
  $("#productsGrid").innerHTML = products.map(p => cardTemplate(p, "producto")).join("");
